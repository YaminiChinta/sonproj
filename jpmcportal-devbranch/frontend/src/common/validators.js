...
export const emailValidator = (email) => {
  ...
  } else if (!new RegExp(/^[a-z0-9_.#$]+@gmail.com+$/).test(email)) {
    return (<div className="invalid-feedback d-block">This is invalid email format</div>);
  }
  return "";
};
...
export const idValidator = (cognizantId) => {
  ...
  if (!new RegExp(/^[0-9]+$/).test(cognizantId)) {
    return (<div className="invalid-feedback d-block">This Field must be in digits</div>);
  }else if(!(cognizantId.length >5 && cognizantId.length <10)){
    return (<div className="invalid-feedback d-block">This Field must be between 5 to 10 digits</div>);
  }
  return "";
};
...
export const OTPValidator = (otp) => {
  ...
  if (!new RegExp(/^[0-9]+$/).test(otp)) {
    ...
    return (<div className="invalid-feedback d-block">This Field must be in 6 digits</div>);
  }
  return "";
};

export const passwordValidator = (password) => {
  if(!new RegExp(/^(?=(.*[a-z]){1,})(?=(.*[A-Z]){1,})(?=(.*[0-9]){1,})(?=(.*[!@#$%^&*()\-_+.]){1,}).{8,}$/).test(password)) {
    return (<div className="invalid-feedback d-block">Password must be have at least 8 characters long, contain at least one special character and one number and have a mixture of uppercase and lowercase letters.</div>);
  }
